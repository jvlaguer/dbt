...
table_active_pcnt_change_cnt_distinct_host  as (
    ...
    SELECT *,
    (cnt_listings::float/LAG(cnt_listings, 1) OVER (PARTITION BY listing_neighbourhood ORDER BY SUBSTRING(month_year, 4, 4), SUBSTRING(month_year, 1, 2))::float - 1.0) * 100.0 as pcnt_change
...
main as (
    ...
    listing_neighbourhood,
    month_year,
    SUM(CASE WHEN has_availability='t' THEN 1 ELSE 0 END)/COUNT(*) * 100.0 ::float as active_listing_rate,
...
ORDER BY m.listing_neighbourhood, SUBSTRING(m.month_year, 4, 4), SUBSTRING(m.month_year, 1, 2)