...
LEFT JOIN {{ ref('g_dim_lga_suburb') }} s on h.host_neighbourhood = s.suburb_name and fm.scraped_date::timestamp >= s.valid_from and fm.scraped_date::timestamp < coalesce(s.valid_to, '9999-12-31'::timestamp)
GROUP BY 1,2
ORDER BY 1,2
